...
import { AuctionListing, DirectListing, NATIVE_TOKEN_ADDRESS, NFT } from '@thirdweb-dev/sdk'

export default function NFTListInSales({ nftList }: { nftList: (AuctionListing | DirectListing)[] }) {
  ...
  return (
    <div>
      ...
      <div className="flex min-h-[500px] flex-wrap gap-5">
        {nftList?.map(item => {
          return (
            <div className="flex h-60 w-44 flex-col items-center justify-center gap-3 overflow-hidden rounded-md border hover:border-gray-300" key={item.asset.id}>
              <div className="relative block h-full w-full">
                <Image src={item.asset?.image as string} alt={item.asset.name as string} fill sizes="100" />
              </div>
              <h2>
                {item.asset.name}
              </h2>
              <div className="p-1">
                <button onClick={() => sell(item.asset.id)} className="w-32 rounded-l border-r bg-blue-500 p-1 px-2 text-white hover:bg-blue-400">Sell</button>
