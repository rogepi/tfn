  ...
  return (
    <div>
      ...
      <div className="my-8 grid grid-cols-6 gap-5">
        {list.map(item =>
          <Link href={`/nft/${item.metadata.id}`} className="flex h-60 w-full flex-col items-center justify-center
          gap-3 overflow-hidden rounded-md border shadow-md hover:border-gray-300 dark:border-slate-500" key={item.metadata.id}>
            <div className="relative block h-48 w-full flex-1 transition ease-in-out hover:scale-110">
